...
/*
    ...
    written, this example assumes a fan with a tachometer is connected, but 
    pulses generated by other devices can be counted.
    ____________________________________________________________________________
    ...
    1. Pin D4 and Timer0 have a special association defined interally in the
    Arduino, so the pins in this program cannot be changed.

    2. To simplify the hardware setup, the Arduino board's internal pullup
    resistors for counterPin are enabled.

    3. The messages that are printed to serial are in the Strings tab at the top
    of the window.

    4. You may need to change pulsesPerRotation to match the your fan's behavior.
    ____________________________________________________________________________

    Experiments

    - Try different types of devices connected to counterPin (like a pushbutton).
    You can use anything that pulls counterPin to ground and the number of times
    it does will be counted.
    ____________________________________________________________________________
*/
...
// Timer counter is a UInt8 and rolls over after 255. Reading must be fast enough to not loose counts when it rolls over. The higher the readsPerSecond, the more accurate the rpm calculation
...
while(true) {
    ...
    // If it's time to calculate RPMs AND the pulse count is different than last time
    if shouldCalculateRPMs,
